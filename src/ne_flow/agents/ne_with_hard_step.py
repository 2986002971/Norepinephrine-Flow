...
class NE_with_hard_step(flax.struct.PyTreeNode):
    """
    ...
    def high_actor_loss(self, batch, grad_params, rng):
        ...
        current_beta = self.config["high_beta"]
    ...
    def low_actor_loss(self, batch, grad_params, rng):
        ...
        current_beta = self.config["low_beta"]
        weights = jnp.exp(adv * current_beta)
        weights = jnp.clip(weights, max=100.0)
        weights = jax.lax.stop_gradient(weights)
    ...
    @jax.jit
    def total_loss_critic_only(self, batch, grad_params, rng=None):
        info = {}
        ...
        # Zero out actor info to maintain dict structure layout
        info["high_actor/high_actor_loss"] = 0.0
        info["high_actor/high_adv_mean"] = 0.0
        info["high_actor/high_weights"] = 0.0
        info["high_actor/high_beta_curr"] = 0.0
        info["high_actor/high_loss_mult"] = 0.0

        info["low_actor/low_actor_loss"] = 0.0
        info["low_actor/low_adv_mean"] = 0.0
        info["low_actor/low_weights"] = 0.0
        info["low_actor/low_beta_curr"] = 0.0
        info["low_actor/low_loss_mult"] = 0.0

        total = v_loss + c_loss
        return total, info

    @jax.jit
    def update_critic_only(self, batch, step):
        new_rng, rng = jax.random.split(self.rng)

        def loss_fn(grad_params):
            return self.total_loss_critic_only(batch, grad_params, rng=rng)

        new_network, info = self.network.apply_loss_fn(loss_fn=loss_fn)

        # Prevent Adam drift by freezing Actor parameters manually
        old_params = self.network.params
        new_params = dict(new_network.params)
        new_params["modules_high_actor"] = old_params["modules_high_actor"]
        new_params["modules_low_actor"] = old_params["modules_low_actor"]
    ...
    @jax.jit
    def total_loss_actor_only(self, batch, grad_params, rng=None):
        rng = rng if rng is not None else self.rng
        rng, high_rng, low_rng = jax.random.split(rng, 3)
    ...
    def update_actor_only(self, batch, step):
        ...
        def loss_fn(grad_params):
            return self.total_loss_actor_only(batch, grad_params, rng=rng)

        new_network, info = self.network.apply_loss_fn(loss_fn=loss_fn)

        # Prevent Adam drift by freezing Critic parameters manually
        old_params = self.network.params
        new_params = dict(new_network.params)
    ...
    def update(self, batch, step=0):
        # Native Python control flow exactly swapping regimes at hard_step
        hard_step = self.config.get("hard_step", 200000)
        if step <= hard_step:
            return self.update_critic_only(batch, step)
        else:
